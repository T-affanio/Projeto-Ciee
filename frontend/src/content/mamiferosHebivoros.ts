import { Animal } from "@/types/Animal";
export const MamiferosHerbivoros: Animal[] =[
    {
        name:'Girafa',
        images:[
            '/Image/animal/girafa/girafa.webp',
            '/Image/animal/girafa/girafa 1.jpg',
            '/Image/animal/girafa/girafa 3.jpg',
        ],
        description:'As girafas são mamíferos nativos da África, conhecidos por suas longas pernas e pescoço, que as tornam os animais terrestres mais altos do mundo. Elas podem atingir até 5,5 metros de altura. Sua pelagem é marcada por manchas irregulares, que variam entre o marrom e o amarelo. As girafas se alimentam principalmente de folhas de árvores, especialmente as acácias, usando sua língua longa e flexível para alcançá-las. Elas têm uma expectativa de vida de cerca de 25 anos na natureza e são animais sociais, vivendo em grupos conhecidos como torres.',
        curiosities:'Sono raro: Girafas dormem apenas de 10 a 30 minutos por vez, e geralmente ficam em pé enquanto descansam. Elas não têm um ciclo de sono profundo como os humanos, e dormem em intervalos curtos durante o dia ou à noite.',
        habitat:' As girafas habitam principalmente as savanas, planícies e áreas abertas da África subsaariana. Elas preferem locais com árvores esparsas, como as acácias, onde podem se alimentar das folhas. As girafas também são encontradas em áreas com arbustos, onde podem procurar por alimentos. Elas evitam florestas densas, pois seu grande porte não é ideal para essas regiões.',
         age:'16 anos',
        origin:'África'
    },
]